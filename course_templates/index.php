...
if(!$cateid){
    $cateid = $DB->get_field('course_categories','id',array('name'=>'course templates'));
}
...
echo "<div class='custom-course-template'>";
    switch ($step) {
        case 2:
    ...
    $PAGE->requires->js_call_amd('local_course_templates/message');
    if (!$step) {
        echo "<div class='custom-course-generaltable1'>";

          $sql = "SELECT id,name from {course_categories}";
  $allavailableprograms = $DB->get_records_sql($sql);
  $programs = array('Pick a Category');
  foreach ($allavailableprograms as &$allavailableprograms) {
    $programs[$allavailableprograms->id] = $allavailableprograms->name;
  }
            $htmlstepper = '';
            ...
            $htmlstepper .= '</div>';
            echo $htmlstepper;
            echo$OUTPUT->single_select(new moodle_url('', array()), 'cateid', $programs, $cateid, '', array(), array('label' => ''));
            echo html_writer::tag('p', html_writer::tag('strong', get_string('choosetemplate', 'local_course_templates')));
            $perpage = 10;
            echo get_template_list_form($cateid,$page,$perpage);
            $url = new moodle_url('/local/course_templates/index.php?cateid='.$cateid);
            $totaltemplates = get_template_counts($cateid);
            echo $OUTPUT->paging_bar($totaltemplates, $page, $perpage, $url);
        echo '</div>';
    }
    else if ($step == 2) {
        echo "<div class='custom-course-generaltable2'>";
            $htmlstepper = '';
            $htmlstepper .= '<div class="bs-stepper">';
            $htmlstepper .= '<div class="bs-stepper-header" role="tablist">';
        ...
        echo "<div class='custom-course-generaltable3'>";
        ...
        echo "<div class='custom-course-generaltable4'>";
            $status = $coursestatus;
