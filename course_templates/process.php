...
$categoryid = optional_param('cateid', 7, PARAM_INT);
...
if (@isset($res['id'])) {
    try {
        ...
        $old_course_starttime = $oldcourse->startdate;
        $difference = $startdatetime - $old_course_starttime;
        $course = $DB->get_record('course', array('id' => $res['id']));

        if (!empty($startdatetime)) {
            $course->startdate = $startdatetime;
            $course->enddate = $oldcourse->enddate+$difference;
        ...
        if (!empty($location)) {
            ...
            $DB->update_record('course_format_options', $eventoption);
             ...
             //Changing restrict Access for sections
            $courseid = $res['id'];  //new created courseid
            $records = $DB->get_records('course_modules', array('course' => $courseid));
            foreach ($records as &$record) {
                if (is_null($record->availability))
                    continue;
                $record->availability = json_decode($record->availability);
                for($i = 0; $i < count($record->availability->c); $i++){
                if ($record->availability->c[$i]->t) {
                    $record->availability->c[$i]->t += $difference;
                }
            }
                $record->availability = json_encode($record->availability);
        ...
        rebuild_course_cache($res['id'], true);
        //Enrolment deletion
        $manualplugin = enrol_get_plugin('manual');
        $minstance = $DB->get_record('enrol', array('courseid' => $course->id, 'enrol' => 'manual'));
        $manualplugin->delete_instance($minstance);
        $manualplugin->add_default_instance($course);
    } catch (\Exception $e) {
        \core\notification::error($e->getMessage());
        exit(json_encode(array('status' => 0)));
    }
        //custom code for checking new course creation
        $created_from_name = $DB->get_field('course','fullname', array('id' => $courseid));
        $custom_data = new stdClass();
        $custom_data->created_courseid = $course->id;
        $custom_data->type = 'Template Course';
        $custom_data->created_from = $courseid;
        $custom_data->created_from_name = $created_from_name;
        $custom_data->timecreated = time();
        $custom_report_template = $DB->insert_record('custom_report_template',$custom_data);
