 ...
 */
function get_template_list($namecategoryid, $page,$perpage) {
    global $DB;
    $params = array();
 ...
 * @throws dml_exception
 */
function get_template_list_form($cateid = null,$page=0,$perpage=10) {
    global $CFG, $USER;
    ...
    $rows = get_template_list($cateid,$page,$perpage);
    ...
    foreach ($rows as $row) {
        ...
        $html = '<a href="'.$courseurl.'" class="template_preview" target="_blank">Preview</a>';
        $data[] = $html;
 ...
 */
function get_template_counts($cateid) {
    global $DB;
    $params = array();
//    $namecategoryid = get_config('local_course_templates', 'namecategory');

    $sql = "select count(id) as count, fullname from {course} where category = (select id from {course_categories} where id='$cateid')";
...
function get_template_setting_form($cid, $categoryid, $cateid = null) {
    ...
    $output .= '<div id="course_templates_validation_error_message" data-validation-message="'
            ...
            . '"></div>';
    $output .= '<div class="custom_note font-13  grey-text-color">
    <span class="d-block font-w-600  lightgrey-text">What is duplicated?</span>
        All the course contents, including the live classes, assignments and quizzes would be copied to the duplicate course.
    <span class="d-block font-w-600 mt-3 lightgrey-text">What is not duplicated?</span>
        The student enrollments would not be carried forward.</br>

    <span class="d-block font-w-600 mt-3 lightgrey-text">Important note</span>
Please ensure that after creating the duplicate course, you edit the dates for live classes, assignments, quizzes, labs etc. so that it is in accordance with the requirements of the new course. </div>';
