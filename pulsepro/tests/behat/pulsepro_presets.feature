...
Feature: Create pulse using preset with pro features.
  ...
  Scenario: Created preset should displayed in preset list.
    Given I navigate to "Plugins > Local plugins > Manage Presets" in site administration
    ...
    And I upload "/local/pulsepro/assets/preset-demo-3.mbz" file to "Preset Template" filemanager
    ...
    Then I set the field with xpath "//div[@class='preset-config-params']//select[@id='id_reactiontype']" to "Rate"
    And I set the field with xpath "//div[@class='preset-config-params']//select[@id='id_reactiondisplay']" to "Notification and Content"
  ...
  Scenario: Enable and disable preset visiblity.
    ...
    And I click on "Add an activity or resource" "button"
    And I click on "Pulse" "link"
    And I should not see "Demo preset 1" in the ".preset-title" "css_element"
    And I navigate to "Plugins > Local plugins > Manage Presets" in site administration
    And I click on "Disabled" "text" in the "#list-pulse-presets_r0_c4" "css_element"
    And I am on "Test" course homepage with editing mode on
    And I click on "Add an activity or resource" "button"
    And I click on "Pulse" "link"
    And I should see "Demo preset 1" in the ".preset-title" "css_element"

  @javascript
  Scenario: Delete preset.
    Given I navigate to "Plugins > Local plugins > Manage Presets" in site administration
    And I click on ".fa-trash" "css_element" in the "#list-pulse-presets_r0_c6" "css_element"
    Then ".confirmation-dialogue" "css_element" should be visible
    And I should see "Are you sure! do you want to delete the preset" in the ".confirmation-message" "css_element"
    When I click on "input.btn-primary" "css_element" in the ".confirmation-buttons" "css_element"
    And I should see "Preset deleted successfully"
    Then I should not see "Demo preset 1"
    And I am on "Test" course homepage with editing mode on
    And I click on "Add an activity or resource" "button"
    And I click on "Pulse" "link"
    And I should not see "Demo preset 1" in the ".preset-title" "css_element"
